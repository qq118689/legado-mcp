...
public class ApkMcpClient {
    ...
    public boolean connect() {
        ...
        try {
            LogUtils.i(TAG, "正在连接 MT APK MCP: " + mcpUrl);
            ...
            if (initResp == null) {
                LogUtils.w(TAG, "initialize 失败");
                return false;
            }
            LogUtils.i(TAG, "initialize 成功: " + initResp.toString().substring(0, Math.min(200, initResp.toString().length())));
            ...
            try {
                ...
                LogUtils.w(TAG, "从 initialize 提取工具失败: " + e.getMessage());
            }

            if (tools.length() == 0) {
                LogUtils.w(TAG, "initialize 中未找到 APK 工具");
                return false;
            }

            remoteTools = tools;
            connected = true;
            LogUtils.i(TAG, "连接成功，获取到 " + tools.length() + " 个 APK 工具");
    ...
    public JSONObject callTool(String toolName, JSONObject arguments) {
        ...
        try {
            ...
            LogUtils.e(TAG, "callTool 失败: " + toolName + " - " + e.getMessage());
    ...
    private JSONObject sendMcpRequest(String method, JSONObject params) {
        try {
            ...
            LogUtils.d(TAG, "请求 [" + id + "]: " + method + " (长度=" + bodyStr.length() + ")");
            ...
            if (respCode != 200) {
                LogUtils.w(TAG, method + " HTTP " + respCode + ": " + conn.getResponseMessage());
                return null;
            }
            ...
            if (resp.has("error")) {
                ...
                LogUtils.w(TAG, method + " 返回错误: " + errMsg);
                return null;
            }
            ...
            LogUtils.e(TAG, "sendMcpRequest " + method + " 异常: " + e.getMessage());
            return null;
        }
    }
    ...
    private void sendMcpNotification(String method, JSONObject params) {
        try {
            ...
            LogUtils.w(TAG, "sendMcpNotification 失败: " + e.getMessage());
        }
    }
